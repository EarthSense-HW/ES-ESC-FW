...
// Threads
//
static THD_FUNCTION(stream_debug_thread, arg);
static THD_WORKING_AREA(stream_debug_thread_wa, 256);


static THD_FUNCTION(stream_debug_thread, arg) {
    (void)arg;

    chRegSetThreadName("Terminal_Debug");

    for(;;) {
      if(get_stream_debug()){
        // JB - Place Terminal Debug Stream Code HERE
        commands_printf("Encoder: %.2f,   Abs: %.2f\r\n",
                        (double)encoder_get_angle(),
                        (double)encoder_get_abs_angle()
                        );
      }
...
int main(void) {
	...
	// Threads
	chThdCreateStatic(stream_debug_thread_wa, sizeof(stream_debug_thread_wa), NORMALPRIO, stream_debug_thread, NULL);
