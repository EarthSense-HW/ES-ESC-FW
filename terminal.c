...
void terminal_process_string(char *str)
{
	...
	else if (strcmp(argv[0], "help") == 0)
	{
		...
		// Flux Linkage Tuning
		commands_printf("earthsense_detect_and_apply_foc_linkage [current] [min_rpm] [duty]");
		commands_printf("  Measure and apply motor flux linkage.");

		commands_printf("earthsense_detect_and_apply_foc_linkage_can [current] [min_rpm] [duty]");
		commands_printf("  Measure and apply motor flux linkage.");
		commands_printf("  Fowards command to all VESCs.");

		// Hall Sensor Tuning
		commands_printf("earthsense_detect_and_apply_hall_sensors [current]");
		commands_printf("  Measure and apply hall sensor measurements.");

		commands_printf("earthsense_detect_and_apply_hall_sensors_can [current]");
		commands_printf("  Measure and apply hall sensor measurements.");
		commands_printf("  Fowards command to all VESCs.");
		...
		// Current KP
		commands_printf("earthsense_set_current_kp [current_kp]");
		commands_printf("  Set motor current KP parameter.");

		commands_printf("earthsense_set_current_kp_can [current_kp]");
		commands_printf("  Set motor current KP parameter.");
		commands_printf("  Fowards command to all VESCs.");

		// Current KI
		commands_printf("earthsense_set_current_ki [current_ki]");
		commands_printf("  Set motor current KI parameter.");

		commands_printf("earthsense_set_current_ki_can [current_ki]");
		commands_printf("  Set motor current KI parameter.");
		commands_printf("  Fowards command to all VESCs.");

		// Obersever Gain
		commands_printf("earthsense_set_observer_gain [observer_gain]");
	...
	{
		// Send CAN Packet as Broadcast (ID = 255) [ID]
		int length = 1;
		uint8_t buffer[length];
		buffer[0] = app_get_configuration()->controller_id;
		comm_can_transmit_eid_replace(255 | ((uint32_t)CAN_PACKET_ES_DETECT_APPLY_ALL_R_L << 8), buffer, length, true);
	}
	...
	else if (strcmp(argv[0], "earthsense_detect_and_apply_hall_sensors") == 0)
	{
		...
		float current = -1;
		sscanf(argv[1], "%f", &current);
		earthsense_detect_and_apply_hall_sensors(current);
	}
	else if (strcmp(argv[0], "earthsense_detect_and_apply_hall_sensors_can") == 0)
	{
		...
		// Send CAN Packet as Broadcast (ID = 255) [ID, CURRENT]
		int length = 2;
		uint8_t buffer[length];
		buffer[0] = app_get_configuration()->controller_id;
		buffer[1] = current;
		comm_can_transmit_eid_replace(255 | ((uint32_t)CAN_PACKET_ES_DETECT_APPLY_ALL_HALL << 8), buffer, length, true);
	}
