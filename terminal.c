...
static bool stream_debug = false;
...
void terminal_process_string(char *str) {
	...
	if (strcmp(argv[0], "ping") == 0) {
		...
		if (mcconf.m_sensor_port_mode == SENSOR_PORT_MODE_AS5047_SPI ||
			...
			commands_printf("SPI encoder value: %.2f, errors: %d, error rate: %.3f %%",
	...
	// The help command
		...
		commands_printf("stream_debug");
		commands_printf("  Stream encoder data");
